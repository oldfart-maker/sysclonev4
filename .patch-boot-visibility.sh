...
# 1) Add helper script
mkdir -p tools
cat > tools/seed-boot-visibility.sh <<'EOSH'
#!/usr/bin/env bash
set -Eeuo pipefail
...
EOSH
chmod +x tools/seed-boot-visibility.sh

# 2) Add Makefile target
mf=Makefile
if ! grep -q '^seed-boot-visibility:' "$mf"; then
  cat >> "$mf" <<'EOF'
...
git add tools/seed-boot-visibility.sh "$mf"
git commit -m "boot-visibility: add console+BOOT logging for first-boot/L2/L2.5; new target seed-boot-visibility" || true
git tag -a v4.7.0-boot-visibility -m "Console/BOOT logs for sysclone services" || true
