...
# 1) Payload: the Pi-side Makefile with a few helpful verify/log targets
mkdir -p tools/payloads/usr-local-share
cat > tools/payloads/usr-local-share/sysclone-pi.mk <<'MK'
SHELL := /bin/bash
.ONESHELL:
.DELETE_ON_ERROR:
...
# 2) Payload: simple wrapper to call the Makefile
mkdir -p tools/payloads/usr-local-bin
cat > tools/payloads/usr-local-bin/scpi <<'WRAP'
#!/usr/bin/env bash
set -euo pipefail
exec make -f /usr/local/share/sysclone/pi.mk "${@:-help}"
WRAP
chmod +x tools/payloads/usr-local-bin/scpi

# 3) Host Makefile target to install these onto the card
grep -q '^seed-pi-devtools:' Makefile || cat >> Makefile <<'MF'

# ---------------- Pi devtools (on-target helpers) ----------------
seed-pi-devtools: ensure-mounted ## Install Pi debugging helpers (scpi + pi.mk)
	@set -euo pipefail; \
	  install -D -m 0644 tools/payloads/usr-local-share/sysclone-pi.mk "$(ROOT_MNT)/usr/local/share/sysclone/pi.mk"; \
	  install -D -m 0755 tools/payloads/usr-local-bin/scpi "$(ROOT_MNT)/usr/local/bin/scpi"; \
...
echo "[ok] pi devtools payload + seed target added"
