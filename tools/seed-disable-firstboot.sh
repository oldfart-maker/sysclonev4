...
# Any service referencing firstboot/oem/setup
...
sudo find "$ROOT_MOUNT/etc/profile.d" -maxdepth 1 -type f -iregex '.*first.*boot.*' -print -delete 2>/dev/null || true

stub() {
  local p="$1"
  if [[ -f "$ROOT_MOUNT/$p" ]]; then
    echo "[seed] stubbing $p"
