...
if command -v arch-chroot >/dev/null 2>&1; then
  ...
  install -D -m 0755 "$REPO_ROOT/tools/payloads/usr-local-sbin/sysclone-layer2-install.sh" \
    "$ROOT_MNT/usr/local/sbin/sysclone-layer2-install.sh"
  install -D -m 0644 "$REPO_ROOT/tools/payloads/etc/systemd/system/sysclone-layer2-install.service" \
...
install -D -m 0755 "$REPO_ROOT/tools/payloads/usr-local-sbin/wayland-sanity.sh" \
